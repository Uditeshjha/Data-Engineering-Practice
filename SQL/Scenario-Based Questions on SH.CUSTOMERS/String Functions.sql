...
SELECT cust_id,
       ...
       LENGTH(TRIM(cust_first_name || ' ' || cust_last_name)) AS full_name_length
FROM sh.customers;

-- 5. Replace vowels in customer names with '*'.
SELECT cust_id,
       cust_first_name || ' ' || cust_last_name AS full_name,
       REGEXP_REPLACE(cust_first_name || ' ' || cust_last_name, '[aeiouAEIOU]', '*') AS name_masked
FROM sh.customers;
...
SELECT 
    ...
    -- TO_NUMBER(REPLACE(REGEXP_SUBSTR(cust_income_level, '[0-9,]+'), ',', '')) AS income_value
FROM sh.customers;

-- 10. Count how many customers have a 3-letter first name.
SELECT COUNT(*) AS three_letter_first_names
FROM sh.customers
WHERE LENGTH(TRIM(cust_first_name)) = 3;
